...
# Global variable to store selected contact ID
selected_contact_ids = set()

def ExportData():
    global selected_contact_ids
    if not selected_contact_ids:
        tkMessageBox.showwarning('', 'Please Select Contacts to Export!', icon="warning")
    else:
        # Load existing workbook or create a new one if it doesn't exist
        try:
            wb = load_workbook("exported_contacts.xlsx")
            ws = wb.active
        except FileNotFoundError:
            wb = Workbook()
            ws = wb.active
            ws.append(["MemberID", "Firstname", "Lastname", "Gender", "Age", "Address", "Contact", "Email", "Notes"])
        
        conn = sqlite3.connect("pythontut.db")
        cursor = conn.cursor()
        for contact_id in selected_contact_ids:
            cursor.execute("SELECT * FROM `member` WHERE `mem_id` =?", (contact_id,))
            contact_data = cursor.fetchone()
            ws.append(contact_data)
        conn.close()  # Close connection after fetching all data
        # Save all selected contacts in the same Excel file
        wb.save("exported_contacts.xlsx")
        tkMessageBox.showinfo('', 'Contacts exported successfully!')

def OnSelected(event):
    global selected_contact_ids
    curItem = tree.focus()
    selected_contact_ids = [tree.item(curItem, "values")[0]]
    if event.num == 1:
        UpdateData()
    elif event.num == 3:
        ExportData()
...
def Database():
    ...
    cursor = conn.cursor()
    cursor.execute(
        "CREATE TABLE IF NOT EXISTS `member` (mem_id INTEGER NOT NULL  PRIMARY KEY AUTOINCREMENT, firstname TEXT, lastname TEXT, gender TEXT, age TEXT, address TEXT, contact TEXT, Email TEXT, notes TEXT)")
    cursor.execute("SELECT * FROM `member` ORDER BY `lastname` ASC")
    fetch = cursor.fetchall()
    for data in fetch:
        tree.insert('', 'end', values=(data))
...
def SubmitData():
    if FIRSTNAME.get() == "" or LASTNAME.get() == "" or GENDER.get() == "" or AGE.get() == "" or ADDRESS.get() == "" or CONTACT.get() == "" or EMAIL.get() == "":
        ...
        conn = sqlite3.connect("pythontut.db")
        cursor = conn.cursor()
        cursor.execute("INSERT INTO `member` (firstname, lastname, gender, age, address, contact, email, notes) VALUES(?, ?, ?, ?, ?, ?, ?, ?)",
                       (str(FIRSTNAME.get()), str(LASTNAME.get()), str(GENDER.get()), int(AGE.get()), str(ADDRESS.get()), str(CONTACT.get()), str(EMAIL.get()), str(NOTES.get())))
        conn.commit()
        cursor.execute(
...
def UpdateData():
    if FIRSTNAME.get() == "" or LASTNAME.get() == "" or GENDER.get() == "" or AGE.get() == "" or ADDRESS.get() == "" or CONTACT.get() == "" or EMAIL.get() == "":
        ...
        conn = sqlite3.connect("pythontut.db")
        cursor = conn.cursor()
        cursor.execute("UPDATE `member` SET `firstname` = ?, `lastname` = ?, `gender` = ?, `age` = ?, `address` = ?, `contact` = ?, `email` = ?, `notes` = ? WHERE `mem_id` = ?",
                       (str(FIRSTNAME.get()), str(LASTNAME.get()), str(GENDER.get()), str(AGE.get()), str(ADDRESS.get()), str(CONTACT.get()), str(EMAIL.get()), str(NOTES.get()), int(mem_id)))
        conn.commit()
        cursor.execute(
            "SELECT * FROM `member` ORDER BY `lastname` ASC")
        fetch = cursor.fetchall()
...
def OnSelected(event):
    global mem_id, UpdateWindow, selected_contact_ids
    curItem = tree.focus()
    ...
    if event.num == 1:
        ...
        Male = Radiobutton(
            RadioGroup, text="Male", variable=GENDER, value="Male",  font=('arial', 14)).pack(side=LEFT)
        Female = Radiobutton(
            RadioGroup, text="Female", variable=GENDER, value="Female",  font=('arial', 14)).pack(side=LEFT)

        # ===================LABELS==============================
        lbl_title = Label(FormTitle, text="Update Contacts",
        ...
        lbl_age = Label(ContactForm, text="Age",
                        font=('arial', 14), bd=5)
        ...
        lbl_email = Label(ContactForm, text="Email",
                        font=('arial', 14), bd=5)
        lbl_email.grid(row=6, sticky=W)
        lbl_notes = Label(ContactForm, text="Notes",
                        font=('arial', 14), bd=5)
        lbl_notes.grid(row=7, sticky=W)
        ...
        notes = Entry(ContactForm, textvariable=NOTES,  font=('arial', 14))
        notes.grid(row=7, column=1)

        # ==================BUTTONS==============================
        btn_updatecommand = Button(ContactForm, text="Update",
                                width=50, command=UpdateData)
        btn_updatecommand.grid(row=8, columnspan=2, pady=10)
...
def AddNewWindow():
    ...
    lbl_email.grid(row=6, sticky=W)
    lbl_notes = Label(ContactForm, text="Notes", font=('arial', 14), bd=5)
    lbl_notes.grid(row=7, sticky=W)
    ...
    # ==================BUTTONS==============================
    btn_addcommand = Button(ContactForm, text="Save", width=50, command=SubmitData)
    btn_addcommand.grid(row=8, columnspan=2, pady=10)
...
Top.pack(side=TOP)
Mid = Frame(root, width=700, bg="#6666ff")
Mid.pack(side=TOP)
...
MidRightPadding = Frame(Mid, width=100, bg="#6666ff")
MidRightPadding.pack(side=RIGHT)
TableMargin = Frame(root, width=500)
TableMargin.pack(side=TOP)
# ============================LABELS=====================================
lbl_title = Label(Top, width=700, font=('arial', 18), text="Contact List")
lbl_title.pack(fill=X)

# ============================ENTRY=======================================
search = Entry(MidLeft, textvariable=SEARCH, font=('arial', 14))
search.pack()
# ============================BUTTONS=====================================
btn_add = Button(MidLeft, text="+ ADD NEW", bg="#66ff66", command=AddNewWindow)
btn_add.pack()
btn_search = Button(MidLeft, text="SEARCH",
                    bg="yellow", command=SearchData)
btn_search.pack()
btn_export = Button(MidRight, text="EXPORT",
                    bg="blue", command=ExportData)
btn_export.pack(side=RIGHT)
btn_delete = Button(MidRight, text="DELETE",
...
tree = ttk.Treeview(TableMargin, columns=("MemberID", "Firstname", "Lastname", "Gender", "Age", "Address", "Contact", "Email", "Notes"),
